...
public class ChatUserItemController implements Controller<Node> {
  ...
  private final ChangeListener<URL> avatarChangeListener;
  ...
  private final WeakChangeListener<URL> weakAvatarChangeListener;
  ...
  public ChatUserItemController(PreferencesService preferencesService, AvatarImageService avatarImageService,
    ...
    playerChangeListener = (observable, oldValue, newValue) -> JavaFxUtil.runLater(() -> {
      if (oldValue != null) {
        ...
        JavaFxUtil.removeListener(oldValue.clanTagProperty(), weakClanChangeListener);
        ...
        weakClanChangeListener.changed(oldValue.clanTagProperty(), oldValue.getClanTag(), null);
      ...
      if (newValue != null) {
        ...
        JavaFxUtil.addListener(newValue.clanTagProperty(), weakClanChangeListener);
        ...
        weakClanChangeListener.changed(newValue.clanTagProperty(), null, newValue.getClanTag());
  ...
  private void updateColor() {
    ...
    chatUser.getPlayer().ifPresent(player -> {
      if (player.getSocialStatus() == SocialStatus.SELF) {
        usernameLabel.getStyleClass().add(SocialStatus.SELF.getCssClass());
        clanMenu.getStyleClass().add(SocialStatus.SELF.getCssClass());
      }
    });
  ...
  private void setAvatarUrl(@Nullable URL avatarUrl) {
    updateAvatarTooltip();
    if (avatarUrl == null) {
      avatarImageView.setVisible(false);
    } else {
      // Loading the avatar image asynchronously would be better but asynchronous list cell updates don't work well
      avatarImageView.setImage(avatarImageService.loadAvatar(avatarUrl));
...
//      i18n.get("userInfo.idleTimeFormat", timeService.timeAgo(player.getIdleSince()))));
  ...
  private void updateClanMenu(Player player) {
    clanService.getClanByTag(player.getClanTag())
  ...
  private void updateClanMenu(Optional<Clan> optionalClan) {
    ...
    if (currentPlayer.getId() != clan.getLeader().getId()
      ...
      messageLeaderItem.setOnAction(event -> eventBus.post(new InitiatePrivateChatEvent(clan.getLeader().getDisplayName())));
