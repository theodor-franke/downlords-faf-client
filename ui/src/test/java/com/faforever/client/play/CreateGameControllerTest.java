...
public class CreateGameControllerTest extends AbstractPlainJavaFxTest {
  ...
  private ObservableList<FaMap> mapList;
  ...
  public void testMapSearchTextFieldFilteringEmpty() {
    ...
    assertThat(instance.filteredFaMaps.getSource(), empty());
  }
  ...
  public void testSelectLastMap() {
    ...
    mapList.addAll(map1, lastFaMap);

    WaitForAsyncUtils.asyncFx(() -> instance.initialize());
    WaitForAsyncUtils.waitForFxEvents();

    assertThat(instance.mapListView.getSelectionModel().getSelectedItem(), is(lastFaMap));
  }
