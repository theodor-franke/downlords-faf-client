...
public abstract class AbstractChatTabController implements Controller<Tab> {
  ...
  public void initialize() {
    mentionPattern = Pattern.compile("\\b(" + Pattern.quote(userService.getDisplayName()) + ")\\b", CASE_INSENSITIVE);
  ...
  private void loadChatContainer() {
    try (Reader reader = new InputStreamReader(uiService.getThemeFileUrl(UiService.CHAT_CONTAINER).openStream())) {
  ...
  private void appendMessage(ChatMessage chatMessage) throws IOException {
    ...
    if (preferencesService.getPreferences().getChat().getChatFormat() == ChatFormat.COMPACT) {
      themeFileUrl = uiService.getThemeFileUrl(UiService.CHAT_TEXT_COMPACT);
    } else {
      themeFileUrl = uiService.getThemeFileUrl(UiService.CHAT_TEXT_EXTENDED);
    }
  ...
  private void appendChatMessageSection(ChatMessage chatMessage) throws IOException {
    URL themeFileURL;
    if (preferencesService.getPreferences().getChat().getChatFormat() == ChatFormat.COMPACT) {
      themeFileURL = uiService.getThemeFileUrl(UiService.CHAT_SECTION_COMPACT);
    } else {
      themeFileURL = uiService.getThemeFileUrl(UiService.CHAT_SECTION_EXTENDED);
    }
  ...
  private String renderHtml(ChatMessage chatMessage, URL themeFileUrl, @Nullable Integer sectionId) throws IOException {
    ...
    if (playerOptional.isPresent()) {
      ...
      if (StringUtils.isNotEmpty(player.getClanTag())) {
    ...
    html = html.replace("{time}", timeString)
      .replace("{avatar}", Optional.ofNullable(avatarUrl).map(Objects::toString).orElse(""))
      .replace("{displayName}", login)
    ...
    String text = HtmlEscapers.htmlEscaper().escape(chatMessage.getMessage()).replace("\\", "\\\\");
