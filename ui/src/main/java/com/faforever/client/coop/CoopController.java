...
public class CoopController extends AbstractViewController<Node> {
  ...
  private static final Predicate<Game> OPEN_COOP_GAMES_PREDICATE = gameInfoBean ->
    gameInfoBean.getState() == GameState.OPEN
      && KnownFeaturedMod.COOP.getTechnicalName().equals(gameInfoBean.getFeaturedMod());
  ...
  public CoopController(ReplayService replayService, GameService gameService, CoopService coopService,
                        ...
                        MapPreviewService mapPreviewService, UiService uiService, TimeService timeService,
  ...
  private String commaDelimitedPlayerList(CoopResult coopResult) {
    return Arrays.stream(coopResult.getPlayerNames().split(","))
  ...
  private void setSelectedMission(CoopMission mission) {
    Platform.runLater(() -> {
      ...
      mapImageView.setImage(mapPreviewService.loadPreview(mission.getMapFolderName(), PreviewSize.SMALL));
    });
  ...
  public void onPlayButtonClicked() {
    modService.getFeaturedMod(KnownFeaturedMod.COOP.getTechnicalName())
      .thenAccept(featuredModBean -> gameService.hostGame(new HostGameRequest(
        titleTextField.getText(),
        Strings.emptyToNull(passwordTextField.getText()),
        featuredModBean,
        getSelectedMission().getMapFolderName(),
        emptySet(),
