...
import com.faforever.client.play.OpenHostGameEvent;
...
import com.faforever.client.vault.review.ReviewsController.ReviewsSupplier;
...
public class MapDetailController implements Controller<Node> {
  ...
  private FaMap map;
  private ListChangeListener<FaMap> installStatusChangeListener;


  public MapDetailController(MapService mapService, MapPreviewService mapPreviewService, NotificationService notificationService, I18n i18n,
    ...
    installStatusChangeListener = change -> {
      while (change.next()) {
        for (FaMap faMap : change.getAddedSubList()) {
          if (map.getFolderName().equalsIgnoreCase(faMap.getFolderName())) {
            setInstalled(true);
            return;
          }
        }
        for (FaMap faMap : change.getRemoved()) {
          if (map.getFolderName().equals(faMap.getFolderName())) {
            setInstalled(false);
            return;
          }
        }
  ...
  private void renewAuthorControls() {
    ...
    boolean viewerIsAuthor = map.getAuthor() != null && String.valueOf(player.getDisplayName()).equals(map.getAuthor());
  ...
  public void setMap(FaMap map) {
    ...
    if (map.getLargeThumbnailUrl() != null) {
      thumbnailImageView.setImage(mapPreviewService.loadPreview(map, PreviewSize.LARGE));
    } else {
    ...
    reviewsController.setReviewsSupplier(new MapReviewsSupplier());

    reviewService.findOwnMapReview(map.getId())
      .thenAccept(reviewsController::setOwnReview);
  ...
  private void onDeleteReview(Review review) {
    reviewService.deleteMapVersionReview(review)
      .thenRun(() -> Platform.runLater(() -> reviewsController.setOwnReview(Optional.empty())))
  ...
  private void onSendReview(Review review) {
    ...
    reviewService.saveMapVersionReview(review, map.getId())
      .thenRun(() -> reviewsController.setOwnReview(Optional.of(review)))
      // TODO display error to user
      .exceptionally(throwable -> {
  ...
  public void onCreateGameButtonClicked() {
    eventBus.post(new OpenHostGameEvent(map.getFolderName()));
  }
  ...
  private class MapReviewsSupplier implements ReviewsSupplier {
    @Override
    public CompletionStage<List<Review>> getNonEmptyReviews(int count, int page) {
      return reviewService.findNonEmptyMapReviews(map.getId(), count, page);
    }

    @Override
    public CompletionStage<Optional<ReviewsSummary>> getReviewsSummary() {
      return reviewService.findModReviewsSummary(map.getId());
    }
  }
}
