...
public class ChatUserContextMenuController implements Controller<ContextMenu> {
  ...
  public ChatUserContextMenuController(PreferencesService preferencesService,
                                       ...
                                       JoinGameHelper joinGameHelper, AvatarService avatarService, AvatarImageService avatarImageService, UiService uiService) {
  ...
  private StringListCell<Avatar> avatarCell() {
    return new StringListCell<>(
      ...
      avatar -> {
        URL url = avatar.getUrl();
        if (url == null) {
          return null;
        }
        return new ImageView(avatarImageService.loadAvatar(url));
      });
  }
  ...
  public void setChatUser(ChatChannelUser chatUser) {
    ...
    playerChangeListener = (observable, oldValue, newValue) -> {
      ...
      if (newValue.getSocialStatus() == SocialStatus.SELF) {
        loadAvailableAvatars(newValue);
      }

      kickItem.visibleProperty().bind(newValue.socialStatusProperty().isNotEqualTo(SocialStatus.SELF));
      banItem.visibleProperty().bind(newValue.socialStatusProperty().isNotEqualTo(SocialStatus.SELF));
      moderatorActionSeparator.visibleProperty().bind(newValue.socialStatusProperty().isNotEqualTo(SocialStatus.SELF));
      sendPrivateMessageItem.visibleProperty().bind(newValue.socialStatusProperty().isNotEqualTo(SocialStatus.SELF));
      addFriendItem.visibleProperty().bind(
        newValue.socialStatusProperty().isNotEqualTo(SocialStatus.FRIEND).and(newValue.socialStatusProperty().isNotEqualTo(SocialStatus.SELF))
      );
      removeFriendItem.visibleProperty().bind(newValue.socialStatusProperty().isEqualTo(SocialStatus.FRIEND));
      addFoeItem.visibleProperty().bind(newValue.socialStatusProperty().isNotEqualTo(SocialStatus.FOE).and(newValue.socialStatusProperty().isNotEqualTo(SocialStatus.SELF)));
      removeFoeItem.visibleProperty().bind(newValue.socialStatusProperty().isEqualTo(SocialStatus.FOE));

      joinGameItem.visibleProperty().bind(newValue.socialStatusProperty().isNotEqualTo(SocialStatus.SELF)
      ...
      inviteItem.visibleProperty().bind(newValue.socialStatusProperty().isNotEqualTo(SocialStatus.SELF)
  ...
  private void loadAvailableAvatars(Player player) {
    avatarService.getAvailableAvatars(player.getId()).thenAccept(avatars -> {
      ObservableList<Avatar> items = FXCollections.observableArrayList(avatars);
      items.add(0, new Avatar(null, null, i18n.get("chat.userContext.noAvatar")));

      URL currentAvatarUrl = player.getAvatarUrl();
      Platform.runLater(() -> {
        avatarComboBox.getItems().setAll(items);
        avatarComboBox.getSelectionModel().select(items.stream()
          .filter(avatar -> Objects.equals(Objects.toString(avatar.getUrl(), null), currentAvatarUrl))
          .findFirst()
          .orElse(null));
        ...
        avatarComboBox.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
          ...
          player.setAvatarUrl(newValue == null ? null : newValue.getUrl());
  ...
  public void onAddFriendSelected() {
    ...
    if (player.getSocialStatus() == SocialStatus.FOE) {
