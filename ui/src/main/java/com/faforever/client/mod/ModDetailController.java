...
public class ModDetailController implements Controller<Node> {
  ...
  public ModDetailController(ModService modService, ModThumbnailService modThumbnailService, NotificationService notificationService, I18n i18n,
  ...
  public void setModVersion(ModVersion modVersion) {
    ...
    thumbnailImageView.setImage(modThumbnailService.loadThumbnail(modVersion));
    nameLabel.setText(modVersion.getDisplayName());
    authorLabel.setText(modVersion.getUploader());

    boolean modInstalled = modService.isModInstalled(modVersion.getUuid());
    ...
    setInstalled(modService.isModInstalled(modVersion.getUuid()));
    ...
    reviewsController.setCanWriteReview(modService.isModInstalled(modVersion.getUuid()));
    ...
    reviewsController.setReviewsSupplier(new ModReviewSupplier());
    reviewsController.setOwnReview(modVersion.getReviews().stream()
        .filter(review -> review.getReviewer().getId() == player.getId())
        .findFirst());
  }
  ...
  private class ModReviewSupplier implements ReviewsSupplier {
    @Override
    public CompletionStage<List<Review>> getNonEmptyReviews(int count, int page) {
      return reviewService.findNonEmptyModReviews(modVersion.getMod().getId(), count, page);
    }
