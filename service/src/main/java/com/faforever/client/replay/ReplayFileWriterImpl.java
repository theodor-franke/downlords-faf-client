...
public class ReplayFileWriterImpl implements ReplayFileWriter {
  ...
  public ReplayFileWriterImpl(I18n i18n, ClientProperties clientProperties, PreferencesService preferencesService, ObjectMapper objectMapper) {
    this.i18n = i18n;
  ...
  public void writeReplayDataToFile(ByteArrayOutputStream replayData, LocalReplayInfo replayInfo) throws IOException {
    String fileName = String.format(clientProperties.getReplay().getReplayFileFormat(), replayInfo.getId(), replayInfo.getRecorder());
    ...
    try (BufferedWriter writer = Files.newBufferedWriter(replayFile, UTF_8, CREATE_NEW)) {
      ...
      objectMapper.writeValue(writer, replayInfo);
      writer.write('\n');
