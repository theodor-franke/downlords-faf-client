...
public class PircBotXChatService implements ChatService, InitializingBean, DisposableBean {
  ...
  private void onNotice(NoticeEvent event) {
    ...
    if (config.getNickservOnSuccess() != null && StringUtils.containsIgnoreCase(hostmask.getHostmask(), config.getNickservNick())) {
      String message = event.getMessage();
      if (StringUtils.containsIgnoreCase(message, config.getNickservOnSuccess()) || StringUtils.containsIgnoreCase(message, "registered under your account")) {
        onIdentified();
      } else if (message.contains("isn't registered")) {
        pircBotX.sendIRC().message(config.getNickservNick(), String.format("register %s %s@users.faforever.com", getPassword(), userService.getDisplayName()));
  ...
  private void onChatUserLeftChannel(String channelName, String username) {
    ...
    if (userService.getDisplayName().equalsIgnoreCase(username)) {
      synchronized (channels) {
  ...
  private void init() {
    String username = userService.getDisplayName();
  ...
  public CompletableFuture<String> sendMessageInBackground(String target, String message) {
    eventBus.post(new ChatMessageEvent(new ChatMessage(target, Instant.now(), userService.getDisplayName(), message)));
  ...
  public ChatChannelUser getOrCreateChatUser(String username, String channel, boolean isModerator) {
    synchronized (chatChannelUsersByChannelAndName) {
      ...
      if (!chatChannelUsersByChannelAndName.containsKey(key)) {
        ...
        if (chatPrefs.getChatColorMode() == ChatColorMode.CUSTOM && chatPrefs.getUserToColor().containsKey(userToColorKey(username))) {
  ...
  public void onPlayerOnline(PlayerOnlineEvent event) {
    ...
    synchronized (channels) {
      List<ChatChannelUser> channelUsers = channels.values().stream()
          .map(channel -> chatChannelUsersByChannelAndName.get(mapKey(player.getDisplayName(), channel.getName())))
