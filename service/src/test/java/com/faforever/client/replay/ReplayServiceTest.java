...
import com.faforever.client.replay.LocalReplayInfo.FeaturedMod;
...
public class ReplayServiceTest {
  ...
  public void testRunScFaReplayFile() throws Exception {
    ...
    verify(gameService).runWithReplay(any(), eq(null), eq("faf"), eq(3599), eq(emptySet()), eq(TEST_MAP_NAME));
    verifyZeroInteractions(notificationService);
  }
  ...
  public void testRunScFaOnlineReplay() throws Exception {
    ...
    verify(taskService).submitTask(replayDownloadTask);
    verify(gameService).runWithReplay(replayFile, null, "faf", 3599, emptySet(), TEST_MAP_NAME);
    verifyZeroInteractions(notificationService);
  }
