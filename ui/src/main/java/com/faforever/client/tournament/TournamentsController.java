...
public class TournamentsController extends AbstractViewController<Node> {
  ...
  private void displayTournamentItem(Tournament tournament) {
    ...
    if (tournament.getStartingAt() != null) {
      startingDate = MessageFormat.format(i18n.get("dateWithTime"), timeService.asDate(tournament.getStartingAt()), timeService.asShortTime(tournament.getStartingAt()));
    }

    String completedDate = i18n.get("tournament.noCompletionDate");
    if (tournament.getCompletedAt() != null) {
      completedDate = MessageFormat.format(i18n.get("dateWithTime"), timeService.asDate(tournament.getCompletedAt()), timeService.asShortTime(tournament.getCompletedAt()));
    }

    Reader reader = new InputStreamReader(TOURNAMENT_DETAIL_HTML_RESOURCE.getInputStream());
    String html = CharStreams.toString(reader).replace("{name}", tournament.getName())
        .replace("{challonge-url}", tournament.getChallongeUrl())
        .replace("{tournament-type}", tournament.getTournamentType())
        .replace("{starting-date}", startingDate)
        .replace("{completed-date}", completedDate)
        .replace("{description}", tournament.getDescription())
        .replace("{tournament-image}", tournament.getLiveImageUrl())
